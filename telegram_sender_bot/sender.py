...
class TelegramSender:
    ...
    async def load_accounts(self):
        ...
        for account in accounts:
            try:
                session_path = os.path.join(self.session_folder, account['session_name'])
    ...
    async def send_private_message(self, account_id: int, target_identifier: str, message: str, 
        ...
        try:
            ...
            try:
                if target_identifier.isdigit():
                    ...
                    target_user = await client.get_users(target_identifier.replace('@', ''))
            except Exception:
            ...
            self.logger.info(f"Message sent to {target_user.username or target_user.id}")
            ...
            self.db.update_message_status(message_id, 'failed', 'محدودیت ارسال پیام (Flood)')
            return {'success': False, 'error': 'محدودیت ارسال پیام. لطفا کمی صبر کنید'}
        except errors.UserIsBlocked:
            self.db.update_message_status(message_id, 'failed', 'کاربر شما را مسدود کرده')
            return {'success': False, 'error': 'کاربر شما را مسدود کرده است'}
        except errors.ChatWriteForbidden:
            self.db.update_message_status(message_id, 'failed', 'امکان ارسال پیام وجود ندارد')
            return {'success': False, 'error': 'امکان ارسال پیام به این کاربر وجود ندارد'}
        except Exception as e:
            self.db.update_message_status(message_id, 'failed', str(e))
            self.logger.error(f"Error sending message: {str(e)}")
            return {'success': False, 'error': str(e)}
    
    async def bulk_send(self, account_id: int, target_list: List[str], message: str, 
                       delay_range: tuple = (2, 5)) -> Dict:
    ...
    async def search_users(self, account_id: int, query: str, limit: int = 10) -> List[Dict]:
        """Search for users"""
        try:
            if account_id not in self.clients:
                return []
