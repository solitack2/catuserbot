...
class DatabaseManager:
    ...
    def update_account_status(
        self, account_id: int, status: str, error_msg: str = None
    ):
    ...
    def assign_account_to_category(self, account_id: int, category_id: int):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE accounts SET category_id = ? WHERE id = ?",
    ...
    def assign_proxy_to_account(self, account_id: int, proxy_id: int):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE accounts SET proxy_id = ? WHERE id = ?", (proxy_id, account_id)
            )
            conn.commit()
