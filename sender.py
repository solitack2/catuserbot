...
class TelegramSender:
    ...
    async def load_account(self, account: Dict) -> Optional[Client]:
        ...
        try:
            ...
            if account.get("proxy_id"):
                ...
                proxy = next(
                    (p for p in proxies if p["id"] == account["proxy_id"]), None
                )
            ...
            self.db.update_account_status(
                account["id"], "flood", f"Flood wait until {wait_until}"
            )
    ...
    async def extract_members(
        self, account_id: int, chat_identifier: str
    ) -> List[Dict]:
        ...
        try:
            ...
            if account_id not in self.clients:
                account = next(
                    (acc for acc in self.db.get_accounts() if acc["id"] == account_id),
                    None,
                )
                if not account:
    ...
    async def send_message(
        self, client: Client, user_id: int, message: str, media_path: str = None
    ) -> Dict:
    ...
    async def send_bulk_messages(
        ...
        try:
            ...
            account_rest = int(
                settings.get("account_rest", Config.DEFAULT_ACCOUNT_REST)
            )
            ...
            for member_id in member_ids:
                ...
                # Log to database
                account_id = list(self.clients.keys())[
                    list(self.clients.values()).index(client)
                ]
                status = "sent" if result["status"] == "success" else "failed"
                self.db.log_message(
                    account_id, member_id, message, status, category_id, media_path
                )
                ...
                if messages_sent_per_client[current_client_idx] >= send_limit:
                    self.logger.info(
                        f"Account reached limit, resting for {account_rest}s"
                    )
                    # Move to next client
                    current_client_idx = (current_client_idx + 1) % len(clients)

            self.logger.info(
                f"Bulk sending completed: {len(results)} messages processed"
            )
            return results
        ...
        try:
            ...
            for account in accounts:
                if not account.get("proxy_id"):
                    ...
                    if available_proxy:
                        self.db.assign_proxy_to_account(
                            account["id"], available_proxy["id"]
                        )
                        assigned_count += 1
    ...
    async def get_account_analytics(
        ...
        try:
            ...
            account_stat = next(
                (stat for stat in stats if stat["id"] == account_id), None
            )
            ...
            success_rate = (
                (successful / total_messages * 100) if total_messages > 0 else 0
            )
