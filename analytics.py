...
class AdvancedAnalytics:
    ...
    def _create_daily_chart(self, daily_stats: List[Tuple]) -> io.BytesIO:
        ...
        ax.plot(
            dates, failed_messages, label="Failed", linewidth=2, marker="^", color="red"
        )
    ...
    def _create_success_rate_chart(self, daily_stats: List[Tuple]) -> io.BytesIO:
        ...
        ax.plot(
            dates, success_rates, linewidth=3, marker="o", markersize=8, color="green"
        )
...
"""
        ...
        if stats["accounts"]:
            ...
            for i, account in enumerate(stats["accounts"][:3], 1):
                ...
                rate = account[4] or 0
                report += (
                    f"{i}. {name} ({phone[-4:]}): {total} پیام، {rate:.1f}% موفقیت\n"
                )
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if success_rate < 70:
            recommendations.append(
                "🔸 نرخ موفقیت پایین است. بررسی کنید که آیا اکانت‌ها محدود شده‌اند."
            )
        ...
        flood_errors = sum(
            1 for error in stats["errors"] if "flood" in error[0].lower()
        )
        if flood_errors > 0:
            recommendations.append(
                "🔸 خطاهای Flood شناسایی شد. تاخیر بین پیام‌ها را افزایش دهید."
            )

        # Check account distribution
        if len(stats["accounts"]) == 1:
            recommendations.append(
                "🔸 تنها یک اکانت دارید. افزودن اکانت‌های بیشتر عملکرد را بهبود می‌بخشد."
            )
        ...
        if stats["hourly"]:
            ...
            recommendations.append(
                f"🔸 بیشترین فعالیت در ساعت {peak_hour} است. برای بهینه‌سازی از این ساعت استفاده کنید."
            )

        if not recommendations:
            recommendations.append(
                "🔸 عملکرد شما بسیار خوب است! به همین روند ادامه دهید."
            )
